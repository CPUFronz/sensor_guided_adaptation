...
def train_ofa(opt):
    fix_seeds(opt.seed)
    opt.run_name = 'GTSRB Scale OFA' if opt.nn_scale == 1 else f'GTSRB Scale OFA {opt.nn_scale}x'
    evals = [{'function': evaluate_scale, 'transformation': 'scale'}]
    train_ofa_common(opt, n_neurons=N_NEURONS * opt.nn_scale, n_layers=N_LAYERS, loader=loaders_gtsrb, mod_fun=scale, evaluations=evals)


def train_ft(opt):
    fix_seeds(opt.seed)
    opt.run_name = 'GTSRB Scale Fine-Tune'
    evals = [{'function': evaluate_scale, 'transformation': 'scale'}]
    fine_tune_values = np.linspace(SCALE_MIN, SCALE_MAX, FINE_TUNE_VALUES_SCALE)
    finetune_common(opt, fine_tune_values, loaders_gtsrb, scale, evals)


def train_scn(opt):
    fix_seeds(opt.seed)
    opt.run_name = 'GTSRB Scale SCN'
    evals = [{'function': evaluate_scale, 'transformation': 'scale'}]
    ...
    model = SCN(hin=1, dimensions=opt.D, n_layers=N_LAYERS, n_units=opt.inference_neurons, n_channels=3, n_classes=43, use_batchnorm=opt.batchnorm,  device=opt.device)
    train_ofa_common(opt, opt.inference_neurons, N_LAYERS, loader=loaders_gtsrb, mod_fun=scale, evaluations=evals, alt_model=model)
